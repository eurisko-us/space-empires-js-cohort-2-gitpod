...
for (let i = 0; i < game2.players.length; i++) {
    ...
    if (i == 0) {
        let ship = new Cruiser([3, 0], 1, 1);
        game2.players[i].addShip(ship);
        game2.addToBoard(ship);
    }
    if (i == 1) { 
        let ship = new Scout([3, 1], 2, 1);
        game2.players[i].addShip(ship);
        game2.addToBoard(ship);
    }
...
game2.movementPhase()
assert (game2.players[1].ships[0].coords == [3, 1], "Player 2 can still move if player 1 has moved into their space")
